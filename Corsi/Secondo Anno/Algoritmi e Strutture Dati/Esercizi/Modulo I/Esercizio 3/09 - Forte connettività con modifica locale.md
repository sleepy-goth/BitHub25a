...
Esercizio 3 (Modellazione) — casistica [[Casistiche d'Esame Modulo I]], checklist [[Piano Esame ASD - Modulo I]]; ripasso: [[08 - Grafi e Visite]] (DFS, Kosaraju, SCC).
...
**Modellazione.**
Si usa Kosaraju come subroutine: $G$ è fortemente connesso (FC) se e solo se Kosaraju$(G)$ restituisce un'unica componente fortemente connessa (SCC). Con $|\cdot|$ si indica il numero di SCC restituite.
...
*Algoritmo.* Si controlla dapprima se $G$ è già FC. In caso negativo, per ciascun $x\in C$ si costruisce $G_x$ e si verifica la FC con Kosaraju. Il primo hub che supera il test è la risposta.
...
\begin{algorithmic}
\If{$|\Call{Kosaraju}{G}| = 1$}
  \State \Return "già FC"
\EndIf
\For{ogni $x \in C$}
  \State $G_x \gets \Call{CostruisciGx}{G, x}$
  \If{$|\Call{Kosaraju}{G_x}| = 1$}
    \State \Return $x$
  \EndIf
\EndFor
...
**Complessità:** $O(|C|(n+m))$. Il test iniziale su $G$ costa $O(n+m)$ ed è dominato. Per ciascuno dei $|C|$ hub, CostruisciGx costa $O(n+m)$ (scansione di tutti gli archi) e Kosaraju$(G_x)$ costa $O(n+m)$; il ciclo contribuisce quindi $O(|C|(n+m))$ in totale.
