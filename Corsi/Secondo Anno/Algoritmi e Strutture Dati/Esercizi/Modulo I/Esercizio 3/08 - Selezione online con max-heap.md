...
**Modellazione.**
L'idea chiave è mantenere un max-heap $H$ di taglia esattamente $k$: la radice è sempre il massimo tra i $k$ candidati minimi correnti. Ogni nuovo elemento $a_i$ viene confrontato con $H.\text{max}$:
- se $a_i < H.\text{max}$, allora $a_i$ è migliore del peggior candidato corrente → si sostituisce la radice con $a_i$ (extract-max + insert);
- altrimenti $a_i \ge H.\text{max}$, quindi $a_i$ non può far parte dei $k$ minimi → scartato.

Inizializzazione: si costruisce $H$ coi primi $k$ elementi con una singola chiamata a Build-Heap. Dopo la scansione di tutta la sequenza, $H$ contiene esattamente i $k$ minimi.

```pseudo
\begin{algorithm}
\caption{$k$Minimi($A$, $n$, $k$) → max-heap}
\begin{algorithmic}
\State $H \gets$ \Call{BuildHeap}{$A[1;k]$}
\For{$i \gets k+1$ \To $n$}
  \If{$A[i] < H.\text{max}$}
    \State \Call{ExtractMax}{$H$}
