...
#### Algoritmo Uno
^66510c
Questo approccio è molto numerico e calcola direttamente il numero di Fibonacci:$$\begin{array}{}
...
Ma questo algoritmo è corretto? Beh... No!
...
#### Algoritmo Due
^2bb139
Per evitare i problemi di approssimazione allora possiamo usare precisamente la *Formula di espansione* che abbiamo definito prima nel problema:
...
Questo algoritmo rispetta la tecnica del *divide et impera* (ricorsione). Però rispettando effettivamente la formula di espansione di base, possiamo dire che **l'algoritmo è corretto**. Ma è efficiente?
...
Per i [[Modulo I#^093a4c|Lemmi]] che vedrai di seguito abbiamo quindi che $T(n)\approx F_{n} \approx O(\phi^n)$. Quindi l'algoritmo è *molto lento*.
#### Algoritmo Tre
...
La prima, la seconda e l'ultima riga di codice vengono eseguite una sola volta, mentre la terza e la quarta linea vengono eseguite n volte. Quindi:$$T(n)\leq n+n+3=2n + 3 \approx O(n)$$
L'algoritmo fibonacci3 impiega un tempo lineare (proporzionale a n) rispetto a fibonacci2 che invece impiega un tempo esponenziale. L'altra faccia della medaglia però è lo spazio occupato, che sarà proporzionale all'input.
#### Algoritmo Quattro
Proviamo ad ottimizzare lo spazio occupato dall'algoritmo precedente, eseguendo un salvataggio delle ultime variabili per calcolare il numero di Fibonacci, in quanto non necessitiamo di ogni valore (solo i due precedenti):   
...
def fibonacci4(n: int) -> int:
	...
	return c
```
È nettamente meglio di *fibonacci3*, ma abbastanza? Si può fare di meglio
#### Algoritmo Cinque
Utilizzando il [[Modulo I#^1a3bf9|Terzo Lemma]] possiamo migliorare ulteriormente il calcolo di $F_{n}$ in un tempo inferiore a $O(n)$:
...
#### Algoritmo Sei
Possiamo implementare un calcolo migliorato sull'operazione più "dispendiosa" (cioè il calcolo del numero fino ad n), tramite il [[Modulo I#^0821b0|calcolo di potenza ottimizzato]]:
...
- Il tempo speso dentro `potenzadiMatrice` è costante (Per [[Modulo I#^0821b0|definizione]]).
...
\end{array}$$
Quindi, per $i=\lfloor \log_{2}(n) \rfloor$ si ottiene:$$T(n) \leq c \cdot \lfloor \log_{2}(n) \rfloor + T(1) = O(\log_{2}(n))$$
L'algoritmo più veloce che abbiamo visto!
### Primo Lemma 
...
Per precisare non vi è fibonacci1 in quanto **non corretto**.
...
Mentre per $T(n)=T(n-1)+n$ abbiamo che ogni nodo costa al più $n$ a causa del termine $n$, ma quanti nodi abbiamo?$$n \to n-1 \to n-2 \to n-i \to 2 \to 1$$Beh sempre $n$!
...
Abbiamo $\displaystyle \frac{n}{2}$ nodi e ognuno di loro costa almeno $\displaystyle \frac{n}{2}$, quindi:$$T(n)=\frac{n}{2}\cdot \frac{n}{2}=\frac{n^2}{4}$$
...
Sappiamo sicuramente che ogni chiamata costa uno, quindi ogni nodo costa uno. Sappiamo anche che l'altezza dell'albero è n-1:![[m12.png]]

Quanti nodi ha un albero binario completo di altezza $h$? $$\displaystyle\sum_{i=0}^h 2^i=2^{h-1}-1$$Quindi, grazie all'altezza dell'albero e al numero dei nodi, possiamo dire che $$T(n)=2^{n}-1=\Theta(2^{n})$$

Allora adesso arriviamo a ciò che volevamo analizzare **fibonacci2**:![[m13.png]]
