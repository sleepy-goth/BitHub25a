[[2 - Introduzione informale agli algoritmi|Continua alla lezione successiva.]] 
...
Il **Modello di calcolo** specifica che operazioni possiamo svolgere (es. delle monete la bilancia).

L'algoritmo è la strategia di risoluzione (la strategia di pesatura per il nostro es.), con descrizione comprensibile e compatta. Deve poter eseguire dei passi per risolvere l'algoritmo in una generica istanza. Altri concetti utili da sapere sono i seguenti:
