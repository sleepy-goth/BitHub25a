...
> Albero d-ario, albero con al più d figli, quindi **albero binario** è un albero con due figli per ogni nodo. Un albero d-ario è **completo**: se tutti nodi interni hanno esattamente d figli e le foglie sono tutte allo stesso livello.
...
Possiamo rappresentare la seguente struttura tramite un vettore posizionale.
...
L'algoritmo non fa altro che spostare le chiavi che non rispettano l'heap verso il basso, con la chiave del figlio con valore minore. Essendo che nel caso peggiore deve spostare v alla fine dell'albero, dovrà fare almeno $\log(n)$ operazioni, che valgono ciascuna $O(1)$. Quindi in totale la complessità è $O(\log(n))$.
...
Costruiamo ora lo pseudo-codice dell'algoritmo Heapify, tramite la tecnica divite-et-impera.
...
Perché tramite l'heapsort possiamo ordinare con memoria costante.
#### HeapSort
...
Ordina in loco $O(n\cdot \log(n))$ nel caso peggiore.
